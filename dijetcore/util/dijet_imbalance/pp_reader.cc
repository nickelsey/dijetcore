...
PPReader::PPReader(TTree *tree) : fChain(0) {
  ...
  if (tree == 0) {
    TFile *f = (TFile *)gROOT->GetListOfFiles()->FindObject(
        "trees/y6/y6_grid_16_8/tow_0_track_0.root");
    if (!f || !f->IsOpen()) {
      f = new TFile("trees/y6/y6_grid_16_8/tow_0_track_0.root");
    }
    f->GetObject(
        "LEAD_INIT_R_0.4_alg_2_pt_16_const_eta_1_const_pt_1_MATCH_R_0.4_alg_2_"
        "pt_0_const_eta_1_const_pt_0.2_SUB_INIT_R_0.4_alg_2_pt_8_const_eta_1_"
        "const_pt_1_MATCH_R_0.4_alg_2_pt_0_const_eta_1_const_pt_0.2",
        tree);
  }
  Init(tree);
}
