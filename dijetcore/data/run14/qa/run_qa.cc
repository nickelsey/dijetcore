...
DIJETCORE_DEFINE_string(
    input, "",
    "input root TFile (or txt file with TFile list) with JetTree(s)");
...
DIJETCORE_DEFINE_int(id, -1,
                     "job id: used for batch jobs: keep negative to ignore for "
                     "output file naming");
DIJETCORE_DEFINE_int(nEvents, -1, "number of events to loop over: -1 for all");
DIJETCORE_DEFINE_bool(doTrackQA, true,
                      "flag to turn on/off track QA histograms");
DIJETCORE_DEFINE_bool(doTowerQA, true,
                      "flag to turn on/off tower QA histograms");
DIJETCORE_DEFINE_string(histogramPrefix, "",
                        "can be used to set a prefix for all histogram names");
DIJETCORE_DEFINE_string(
    runIDFile, "",
    "file containing TTree 'runid' that contains list of run 14 runids");
DIJETCORE_DEFINE_string(triggers, "y14vpdmb30",
                        "trigger selection for accepted events - defaults to "
                        "VPD MB 30, a minbias trigger");
...
int main(int argc, char* argv[]) {
  ...
  dijetcore::SetUsageMessage(
      "Run 14 QA routine for run level, event level and track/tower level variables.\n \
    ...
    while (reader->NextEvent()) {
      ...
      if (triggers.size() != 0) {
        ...
        for (auto trigger : triggers)
          if (header->HasTriggerId(trigger)) use_event = true;
