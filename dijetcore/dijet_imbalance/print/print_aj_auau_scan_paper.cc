...
// TGraphErrors *GetSystematic(TH1D *nom, TH1D *var1_a, TH1D *var1_b, TH1D
// *var2_a,
//                             TH1D *var2_b) {
...
// template <class T1, class T2> TH1D *GetErrorFractional(T1 *nom, T2*
// variation, std::string ext = "errfrac") {
...
//                        nom->GetXaxis()->GetXmin(),
//                        nom->GetXaxis()->GetXmax());
...
// template <class T1> TH1D *GetErrorFractional(T1 *nom, TGraphErrors* errors,
...
//   std::vector<string> inputs{FLAGS_auau, FLAGS_ppDir +
//   "/tow_0_track_0.root"}; for (auto &file : inputs) {
//     if (!boost::filesystem::exists(file)) {
//       LOG(INFO) << "input file " << file;
//       LOG(INFO) << "doesn't exist: exiting" << std::endl;
//       return 1;
...
//       keys.push_back(entry.first);
//       parsed_keys[entry.first] =
//       dijetcore::ParseStringToDijetKey(entry.first);
//     }
//   }
...
//             key.sub_init_r == FLAGS_initRadius &&
//             key.lead_match_r == radii[rad] && key.sub_match_r == radii[rad]
//             && key.lead_init_const_pt == constpt[pt] && key.sub_init_const_pt
//             == constpt[pt]) {
//           grid_keys[rad].push_back(key_string);
//           grid_key_params[rad].push_back(key);
//         } else if (!FLAGS_setScanning && key.lead_init_r == radii[rad] &&
...
//   std::vector<std::unordered_map<string, TH2D *>>
//   hard_lead_eta(num_datatypes); std::vector<std::unordered_map<string, TH2D
//   *>> hard_lead_phi(num_datatypes); std::vector<std::unordered_map<string,
//   TH2D *>> hard_lead_pt(num_datatypes);
//   std::vector<std::unordered_map<string, TH2D *>> hard_lead_const(
//       num_datatypes);
//   std::vector<std::unordered_map<string, TH2D *>>
//   hard_lead_rho(num_datatypes); std::vector<std::unordered_map<string, TH2D
//   *>> hard_lead_sig(num_datatypes); std::vector<std::unordered_map<string,
//   TH2D *>> match_lead_eta(num_datatypes);
//   std::vector<std::unordered_map<string, TH2D *>>
//   match_lead_phi(num_datatypes); std::vector<std::unordered_map<string, TH2D
//   *>> match_lead_pt(num_datatypes); std::vector<std::unordered_map<string,
//   TH2D *>> match_lead_const(
//       num_datatypes);
//   std::vector<std::unordered_map<string, TH2D *>>
//   match_lead_rho(num_datatypes); std::vector<std::unordered_map<string, TH2D
//   *>> match_lead_sig(num_datatypes);

//   std::vector<std::unordered_map<string, TH2D *>>
//   hard_sub_eta(num_datatypes); std::vector<std::unordered_map<string, TH2D
//   *>> hard_sub_phi(num_datatypes); std::vector<std::unordered_map<string,
//   TH2D *>> hard_sub_pt(num_datatypes); std::vector<std::unordered_map<string,
//   TH2D *>> hard_sub_const(num_datatypes);
//   std::vector<std::unordered_map<string, TH2D *>>
//   hard_sub_rho(num_datatypes); std::vector<std::unordered_map<string, TH2D
//   *>> hard_sub_sig(num_datatypes); std::vector<std::unordered_map<string,
//   TH2D *>> match_sub_eta(num_datatypes);
//   std::vector<std::unordered_map<string, TH2D *>>
//   match_sub_phi(num_datatypes); std::vector<std::unordered_map<string, TH2D
//   *>> match_sub_pt(num_datatypes); std::vector<std::unordered_map<string,
//   TH2D *>> match_sub_const(
//       num_datatypes);
//   std::vector<std::unordered_map<string, TH2D *>>
//   match_sub_rho(num_datatypes); std::vector<std::unordered_map<string, TH2D
//   *>> match_sub_sig(num_datatypes);
...
//   std::vector<std::unordered_map<string, TH2D *>>
//   lead_dpt_frac(num_datatypes); std::vector<std::unordered_map<string, TH2D
//   *>> sub_dpt_frac(num_datatypes);

//   std::vector<std::unordered_map<string, TH2D *>> hard_aj(num_datatypes);
//   std::vector<std::unordered_map<string, TH2D *>> match_aj(num_datatypes);
//   std::vector<std::unordered_map<string, TH2D *>>
//   hard_aj_test(num_datatypes); std::vector<std::unordered_map<string, TH2D
//   *>> match_aj_test(num_datatypes);

//   std::vector<std::unordered_map<string, TH2D *>> off_axis_aj(num_datatypes);
//   std::vector<std::unordered_map<string, TH2D *>> off_axis_aj_test(
//       num_datatypes);
//   std::vector<std::unordered_map<string, TH2D *>>
//   off_axis_rho(num_datatypes); std::vector<std::unordered_map<string, TH2D
//   *>> off_axis_sig(num_datatypes);
...
//                   copts, out_loc, "off_axis_rho", "", "rho", "event
//                   fraction", "Centrality");
//         Overlay1D(off_axis_sig_cent[data_index][key], refcent_string, hopts,
//                   copts, out_loc, "off_axis_sig", "", "sig", "event
//                   fraction", "Centrality");
//       }

//       if (pp_only_present) {
//         Overlay1D(hard_pp_only_aj_cent[data_index][key], refcent_string,
//         hopts,
//                   copts, out_loc, "pp_only_hard_aj", "", "|A_{J}|",
//                   "event fraction", "Centrality");
//         Overlay1D(match_pp_only_aj_cent[data_index][key], refcent_string,
//         hopts,
//                   copts, out_loc, "pp_only_match_aj", "", "|A_{J}|",
//                   "event fraction", "Centrality");
//         Overlay1D(hard_lead_pp_dpt_cent[data_index][key], refcent_string,
//         hopts,
//                   copts, out_loc, "pp_only_hard_lead_dpt", "", "dp_{T}",
//                   "event fraction", "Centrality");
//         Overlay1D(hard_sub_pp_dpt_cent[data_index][key], refcent_string,
//         hopts,
//                   copts, out_loc, "pp_only_hard_sub_dpt", "", "dp_{T}",
//                   "event fraction", "Centrality");
//         Overlay1D(match_lead_pp_dpt_cent[data_index][key], refcent_string,
//                   hopts, copts, out_loc, "pp_only_match_lead_dpt", "",
//                   "dp_{T}", "event fraction", "Centrality");
//         Overlay1D(match_sub_pp_dpt_cent[data_index][key], refcent_string,
//         hopts,
//                   copts, out_loc, "pp_only_match_sub_dpt", "", "dp_{T}",
//                   "event fraction", "Centrality");
//         Overlay1D(hard_lead_pp_dpt_frac_cent[data_index][key],
//         refcent_string,
...
//                   "dp_{T}/p_{T}", "event fraction", "Centrality");
//         Overlay1D(match_lead_pp_dpt_frac_cent[data_index][key],
//         refcent_string,
//                   hopts, copts, out_loc, "pp_only_match_lead_dpt_frac", "",
//                   "dp_{T}/p_{T}", "event fraction", "Centrality");
//         Overlay1D(match_sub_pp_dpt_frac_cent[data_index][key],
//         refcent_string,
...
//                  copts, out_loc, "aj_match", "", "|A_{J}|", "event
//                  fraction");
//       AjPrintout(match_aj_cent[auau_index][key][i],
...
//                 lead_dr_cent[pp_index][key][i], "Au+Au", "p+p embedded",
//                 hopts, copts, out_loc, "lead_dr", "", "d#phi", "event
//                 fraction");
...
//                 sub_dpt_cent[pp_index][key][i], "Au+Au", "p+p embedded",
//                 hopts, copts, out_loc, "sub_dpt", "", "dp_{T}", "event
//                 fraction");
...
//       p_values_bkg->GetYaxis()->SetBinLabel(j + 1,
//       constpt_string[j].c_str()); ks_values_bkg->GetYaxis()->SetBinLabel(j +
//       1, constpt_string[j].c_str());
...
//         new TCanvas(dijetcore::MakeString("match_tow_sys_unc_pad_",
//         cent).c_str());
...
//         new TCanvas(dijetcore::MakeString("match_trk_sys_unc_pad_",
//         cent).c_str());
...
//         new TCanvas(dijetcore::MakeString("hard_tow_sys_unc_ratio_pad_",
//         cent).c_str());
...
//         new TCanvas(dijetcore::MakeString("hard_trk_sys_unc_ratio_pad_",
//         cent).c_str());
...
//         new TCanvas(dijetcore::MakeString("match_tow_sys_unc_ratio_pad_",
//         cent).c_str());
...
//         new TCanvas(dijetcore::MakeString("match_trk_sys_unc_ratio_pad_",
//         cent).c_str());
...
//         new TCanvas(dijetcore::MakeString("hard_sys_unc_ratio_pad_",
//         cent).c_str());
...
//         new TCanvas(dijetcore::MakeString("match_sys_unc_ratio_pad_",
//         cent).c_str());
...
//         TH1D* hard_tow_p_aj_ratio = GetErrorFractional(aj_hard_pp,
//         hard_tow_p_aj,
//                                                        dijetcore::MakeString("hardtowperr",
//                                                        cent));
...
//         TH1D* hard_tow_m_aj_ratio = GetErrorFractional(aj_hard_pp,
//         hard_tow_m_aj,
//                                                        dijetcore::MakeString("hardtowmerr",
//                                                        cent));
...
//         TH1D* hard_trk_p_aj_ratio = GetErrorFractional(aj_hard_pp,
//         hard_trk_p_aj,
//                                                        dijetcore::MakeString("hardtrkperr",
//                                                        cent));
...
//         TH1D* hard_trk_m_aj_ratio = GetErrorFractional(aj_hard_pp,
//         hard_trk_m_aj,
//                                                        dijetcore::MakeString("hardtrkmerr",
//                                                        cent));
...
//         TH1D* match_tow_p_aj_ratio = GetErrorFractional(aj_match_pp,
//         match_tow_p_aj,
//                                                        dijetcore::MakeString("matchtowperr",
//                                                        cent));
...
//         TH1D* match_tow_m_aj_ratio = GetErrorFractional(aj_match_pp,
//         match_tow_m_aj,
//                                                        dijetcore::MakeString("matchtowmerr",
//                                                        cent));
...
//         TH1D* match_trk_p_aj_ratio = GetErrorFractional(aj_match_pp,
//         match_trk_p_aj,
//                                                        dijetcore::MakeString("matchtrkperr",
//                                                        cent));
...
//         TH1D* match_trk_m_aj_ratio = GetErrorFractional(aj_match_pp,
//         match_trk_m_aj,
//                                                        dijetcore::MakeString("matchtrkmerr",
//                                                        cent));
//         hopts.SetHistogram(match_trk_m_aj_ratio);

//         // // full systematic ratios for hard and matched aj
//         TH1D* hard_aj_sys = GetErrorFractional(aj_hard_pp, hard_sys_error,
//         dijetcore::MakeString("hardtotalerr", cent));
//         hopts.SetHistogram(hard_aj_sys);
//         TH1D* match_aj_sys = GetErrorFractional(aj_match_pp, match_sys_error,
...
//                                   constpt_string.size() - 1, radii[rad],
//                                   constpt[pt], scale_factor);
//         leg11->AddEntry(match_trk_p_aj_ratio, "p+p increased tracking
//         efficiency", "lep")
//             ->SetTextSize(.04 * scale_factor);
//         leg11->AddEntry(match_trk_m_aj_ratio, "p+p decreased tracking
//         efficiency", "lep")
//             ->SetTextSize(.04 * scale_factor);
//         if (pt == constpt.size() - 1 && rad == 0)
//           leg11->Draw();
