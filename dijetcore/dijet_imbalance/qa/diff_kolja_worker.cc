...
int main(int argc, char *argv[]) {
  ...
  // now loop over all found events and see if it exists in both trees
  for (auto &key : all_event_keys) {
    if (!my_events.count(key) && fabs(kolja_events[key].jl.Eta()) < 0.6
        && fabs(kolja_events[key].js.Eta()) < 0.6) {
