## pbs_submit_dijet_imbalance_pythia.py
...
def main(args) :
  ...
  total_pythia_jobs = args.nJobs
  if total_pythia_jobs is 0 :
    return
  
  ## get max number of jobs to be submitted to pbs
  ## at a time
  maxRunningJobs = args.maxRunningJobs
  ...
  jobstatus = [0 for i in range(total_pythia_jobs)]
  ...
  while checkstatus(jobstatus) :
    ...
    while jobsactive >= maxRunningJobs :
    ...
    njobs = maxRunningJobs - jobsactive
    ...
    for i in range(len(jobstatus)) :
      ...
      clargs = '--outputDir=' + args.output + ' --id=' + str(i)
      clargs = clargs + ' --name=' + args.name + ' --constEta=' + args.constEta
      ...
      clargs = clargs + ' --logtostderr' + ' --nEvents=' + str(args.nEvents)
      clargs = clargs + ' --pTHatMin=' + args.pTHatMin
...
if __name__ == "__main__":
  ...
  parser.add_argument('--maxRunningJobs',type=int, default=100, help=' max number of jobs to have in running or queue states')
  ...
  parser.add_argument('--forceConstPtEquality', dest='forceConstPtEquality', action='store_true')
  parser.add_argument('--no-forceConstPtEquality', dest='forceConstPtEquality', action='store_false')
  parser.set_defaults(forceConstPtEquality=True)
  parser.add_argument('--forceConstEtaEquality', dest='forceConstEtaEquality', action='store_true')
  parser.add_argument('--no-forceConstEtaEquality', dest='forceConstEtaEquality', action='store_false')
  parser.set_defaults(forceConstEtaEquality=True)
  parser.add_argument('--forceJetResolutionEquality', dest='forceJetResolutionEquality', action='store_true')
  parser.add_argument('--no-forceJetResolutionEquality', dest='forceJetResolutionEquality', action='store_false')
  parser.set_defaults(forceJetResolutionEquality=True)
  parser.add_argument('--forceMatchJetResolutionEquality', dest='forceMatchJetResolutionEquality', action='store_true')
  parser.add_argument('--no-forceMatchJetResolutionEquality', dest='forceMatchJetResolutionEquality', action='store_false')
