  ...
  for (auto &key : keys) {
    ...
    for (auto type : TYPEMAP) {
      ...
      while (reader.Next()) {
        ...
        hard_rho[data_index][key]->Fill(cent_bin, **jlrho);
        match_rho[data_index][key]->Fill(cent_bin, **jlmrho);
        hard_sigma[data_index][key]->Fill(cent_bin, **jlsig);
        match_sigma[data_index][key]->Fill(cent_bin, **jlmsig);
