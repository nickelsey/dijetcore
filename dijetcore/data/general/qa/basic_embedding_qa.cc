...
int main(int argc, char *argv[]) {
  ...
  reader->GetTowerCuts()->AddBadTowers(FLAGS_towList);
  reader->Init(-1);

  TStarJetPicoReader *mc_reader = new TStarJetPicoReader();
  if (chain != nullptr)
    mc_reader->SetInputChain(mc_chain);
  mc_reader->GetTowerCuts()->AddBadTowers(FLAGS_towList);
  mc_reader->Init(-1);
  ...
  for (int i = 0; i < reader->GetNOfEvents(); ++i) {
    ...
    while (TStarJetPicoPrimaryTrack *track =
               (TStarJetPicoPrimaryTrack *)r_iter.Next()) {
    ...
    while (TStarJetPicoPrimaryTrack *track =
               (TStarJetPicoPrimaryTrack *)mc_iter.Next()) {
