...
DIJETCORE_DEFINE_int(n_events, 100000, "number of pythia events to generate");
DIJETCORE_DEFINE_double(gen_pthat_min, 15.0, "minimum generator pT hat");
DIJETCORE_DEFINE_double(gen_pthat_max, 30.0, "maximum generator pT hat");
DIJETCORE_DEFINE_double(gen_part_pt_min, 0.2, "minimum generator particle pT");
DIJETCORE_DEFINE_double(gen_part_pt_max, 30.0, "maximum generator particle pT");
DIJETCORE_DEFINE_double(gen_part_eta_max, 1.0, "maximum generator particle eta")
DIJETCORE_DEFINE_string(gen_detector_sim_mode, "star", "type of detector simulation to apply - by default, STAR")

DIJETCORE_DEFINE_string(embedInput, "", "input file for embedding Au+Au");
DIJETCORE_DEFINE_int(nEmbed, 5, "number of times to reuse a pythia event");
...
int main(int argc, char* argv[]) {
  ...
  generator.SetPtHatRange(FLAGS_gen_pthat_min,
                          FLAGS_gen_pthat_max);
  generator.SetTrackPtMin(FLAGS_gen_part_pt_min);
  generator.SetTrackPtMax(FLAGS_gen_part_pt_max);
  generator.SetTrackEtaMax(FLAGS_gen_part_eta_max);

  string sim_mode = FLAGS_gen_detector_sim_mode;
  ...
  generator.Initialize(FLAGS_n_events);
  
  // setup the embedding reader
  ...
  TChain* embed_chain = dijetcore::NewChainFromInput(FLAGS_embedInput, "JetTree", FLAGS_id);
  ...
  try {
    while (generator.Next()) {
      ...
      std::string gen_status = generator.GetStatus();
      LOG(INFO) << "Generator status: " << gen_status;
      ...
      for (int emb = 0; emb < FLAGS_nEmbed; ++emb) {
        ...
        int refmult = generator.DetectorParticles().size();
        ...
        std::vector<fastjet::PseudoJet> container = generator.DetectorParticles();
        for (int i = 0; i < container.size(); ++i) {
          fastjet::PseudoJet &sv = container[i];
          ...
          if (sv.user_index() && FLAGS_useEfficiency) {
          ...
          tmpPJ *= scale;
          tmpPJ.set_user_index( sv.user_index());
        ...
        for (auto& result : worker_out) {
          ...
          if (out.found_lead)
          lead_jet_count_dict[key]->Fill(generator.DetectorParticles().size());
          if (out.found_sublead)
          sublead_jet_count_dict[key]->Fill(generator.DetectorParticles().size());
          
          // now fill dijet results
          if (out.found_match) {
            // fill all branches for that key
            run_id_dict[key] = 0;
            event_id_dict[key] = generator.CurrentEvent();
            vz_dict[key] = 0.0;
            refmult_dict[key] = generator.DetectorParticles().size();
            grefmult_dict[key] = 0;
            refmultcorr_dict[key] = refmultcorr;
            grefmultcorr_dict[key] = 0;
            cent_dict[key] = centrality_bin;
            zdcrate_dict[key] = 0.0;
            reactionplane_dict[key] = 0.0;
            nglobal_dict[key] = generator.GenParticles().size();
