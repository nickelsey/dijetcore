...
void Overlay1D(const std::vector<H *> &h, std::vector<std::string> hist_titles,
               ...
               std::string output_name, std::string canvas_title,
               std::string x_axis_label, std::string y_axis_label,
               std::string legend_title = "", bool find_good_range = false) {
...
template <typename H>
void Overlay1D(H *h1, H *h2, double y_min, double y_max, double x_min,
               double x_max, std::string h1_title, std::string h2_title,
               histogramOpts hopts, canvasOpts copts, std::string output_loc,
               std::string output_name, std::string canvas_title,
               std::string x_axis_label, std::string y_axis_label,
               std::string legend_title = "") {
...
template <typename H>
void Overlay1D(H *h1, H *h2, TGraphErrors *sys, double y_min, double y_max,
               double x_min, double x_max, std::string h1_title,
               std::string h2_title, std::string sys_title, histogramOpts hopts,
...
template <class H>
void PrintWithRatio(std::vector<H *> &histograms,
                    std::vector<string> &legend_entries,
...
void PrintWithRatios(H *ref, std::vector<H *> h, std::string ref_name,
                     ...
                     std::string x_axis_label, std::string y_axis_label,
                     std::string legend_title = "",
                     bool use_unit_ratio = true) {
  if (h.size() == 0) {
    std::cerr << "warning: only one histogram, cant take ratios, exiting"
              << std::endl;
    return;
  }
  if (h.size() != h_titles.size()) {
    std::cerr
        << "warning, mismatched number of histograms & histogram names, exiting"
        << std::endl;
    return;
  }
  ...
  for (int i = 0; i < h.size(); ++i) {
    ...
    tmp->GetYaxis()->SetRangeUser(0.6, 1.4);
    tmp->GetYaxis()->SetNdivisions(8);
