...
int main(int argc, char *argv[]) {
  ...
  try {
    while (reader.next()) {
      ...
      eventid = reader.picoDst()->event()->eventId();
      ref = reader.picoDst()->event()->refMult();
      gref = reader.picoDst()->event()->grefMult();
      vz = reader.picoDst()->event()->primaryVertex().Z();
      cent = reader.centrality16();
