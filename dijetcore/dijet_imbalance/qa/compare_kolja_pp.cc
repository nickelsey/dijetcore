...
int main(int argc, char *argv[]) {
  ...
  TTreeReaderValue<int> m_refmult(mytree, "embed_grefmult");
  ...
  using result_container = std::vector<event_info>;
  result_container kolja_events;
  result_container my_events;
  ...
  while (mytree.Next()) {
    ...
    if (tmp.refmult < 269)
      continue;
    all_event_keys.insert(key);
    my_events.push_back(tmp);
  }

  TH1D *k_h_aj = new TH1D("khaj", "", 2800, -0.4999, 0.9);
  TH1D *k_m_aj = new TH1D("kmaj", "", 2800, -0.4999, 0.9);

  TH1D *m_h_aj = new TH1D("mhaj", "", 2800, -0.4999, 0.9);
  TH1D *m_m_aj = new TH1D("mmaj", "", 2800, -0.4999, 0.9);

  TH1D* m_ref = new TH1D("mref", "", 800, 0, 800);
  TH1D* k_ref = new TH1D("kref", "", 800, 0, 800);

  for (auto &e : kolja_events) {
    auto &event = e;

    double hard_aj =
        (event.jl.Pt() - event.js.Pt()) / (event.jl.Pt() + event.js.Pt());
    double match_aj =
        (event.jlm.Pt() - event.jsm.Pt()) / (event.jlm.Pt() + event.jsm.Pt());

    k_h_aj->Fill(hard_aj);
    k_m_aj->Fill(match_aj);
    k_ref->Fill(event.refmult);
  }

  for (auto &e : my_events) {
    auto &event = e;
    ...
    m_h_aj->Fill(hard_aj);
    m_m_aj->Fill(match_aj);
    m_ref->Fill(event.refmult);
  }

  LOG(INFO) << "KS test hard: " << m_h_aj->KolmogorovTest(k_h_aj);
  LOG(INFO) << "KS test matched: " << m_m_aj->KolmogorovTest(k_m_aj);

  LOG(INFO) << "hard aj: ";
  LOG(INFO) << "my mean: " << m_h_aj->GetMean();
  LOG(INFO) << "my std: " << m_h_aj->GetStdDev();
  LOG(INFO) << "my mean err: " << m_h_aj->GetMeanError();
  LOG(INFO) << "my std err: " << m_h_aj->GetStdDevError();
  LOG(INFO) << "kolja mean: " << k_h_aj->GetMean();
  LOG(INFO) << "kolja std: " << k_h_aj->GetStdDev();
  LOG(INFO) << "kolja mean error: " << k_h_aj->GetMeanError();
  LOG(INFO) << "kolja std error: " << k_h_aj->GetStdDevError();

  LOG(INFO) << "matched aj: ";
  LOG(INFO) << "my mean: " << m_m_aj->GetMean();
  LOG(INFO) << "my std: " << m_m_aj->GetStdDev();
  LOG(INFO) << "my mean err: " << m_m_aj->GetMeanError();
  LOG(INFO) << "my std err: " << m_m_aj->GetStdDevError();
  LOG(INFO) << "kolja mean: " << k_m_aj->GetMean();
  LOG(INFO) << "kolja std: " << k_m_aj->GetStdDev();
  LOG(INFO) << "kolja mean error: " << k_m_aj->GetMeanError();
  LOG(INFO) << "kolja std error: " << k_m_aj->GetStdDevError();

  m_h_aj->RebinX(100);
  m_m_aj->RebinX(100);
  k_h_aj->RebinX(100);
  k_m_aj->RebinX(100);
  k_h_aj->SetLineColor(kRed);
  k_m_aj->SetLineColor(kRed);

  m_h_aj->Scale(1.0 / m_h_aj->Integral());
  m_m_aj->Scale(1.0 / m_m_aj->Integral());
  k_h_aj->Scale(1.0 / k_h_aj->Integral());
  k_m_aj->Scale(1.0 / k_m_aj->Integral());

  m_ref->Scale(1.0 / m_ref->Integral());
  k_ref->Scale(1.0 / k_ref->Integral());
  k_ref->SetLineColor(kRed);
  ...
  m_h_aj->Draw();
  k_h_aj->Draw("SAME");
  c.SaveAs("pphardaj.pdf");

  m_m_aj->Draw();
  k_m_aj->Draw("SAME");
  c.SaveAs("ppmatchaj.pdf");
...
// TH1D *unmatched_jlmpt = new TH1D("unmatchedleadmatchpt", ";p_{T}", 50, 0,
// 50); TH1D *unmatched_jsmpt = new TH1D("unmatchedsubmatchpt", ";p_{T}", 50, 0,
// 50); TH1D *unmatched_ref = new TH1D("unmatched_refmult", ";remfult", 100, 0,
// 600); TH1D *unmatched_jleta = new TH1D("unmatchedleadeta", "#eta", 50,
// -1.0, 1.0); TH1D *unmatched_jseta = new TH1D("unmatchedsubeta", "#eta", 50,
// -1.0, 1.0); TH1D *unmatched_jlphi =
//     new TH1D("unmatchedleadphi", "#phi", 50, -TMath::Pi(), TMath::Pi());
// TH1D *unmatched_jsphi =
//     new TH1D("unmatchedsubphi", "#phi", 50, -TMath::Pi(), TMath::Pi());
// TH1D *unmatched_aj_hard = new TH1D("unmatchedajhard", "A_{J}", 20, -0.3,
// 0.9); TH1D *unmatched_aj_matched =
...
// dijetcore::PrettyPrint1D(unmatched_aj_hard, hopts, copts, "unmatched hard
// aj",
...
// TH1D *diff_pt_match_sub = new TH1D("diffptsubmatch", ";p_{T}", 50, -0.5,
// 0.5); TH1D *diff_rho = new TH1D("diffrho", ";d#rho", 50, -30, 30); TH1D
// *diff_rho_norm = new TH1D("diffrhonorm", ";d#rho / #rho", 50, -0.3, 0.3);
...
//       mismatched_jets->Fill(my_events[key].jl.Eta(),
//       my_events[key].jl.Phi());
//     }
//   }
// }
...
//                          "koljas jets p_{T}");
// dijetcore::Print2DSimple(jsmpt, hopts, copts, FLAGS_outputDir,
// "submatchpt2d",
//                          "", "my jets p_{T}", "koljas jets p_{T}");
